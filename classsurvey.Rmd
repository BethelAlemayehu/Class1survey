...
```
```{r, Chunk 1}
# Import CSV and name it C1survey (assumes file is in working directory)
C1survey <- read_csv("C:/Users/Admin/Desktop/Gitdemo/Class 1 Survey Fall 2025 2.csv")
...
```
```{r}
# Task 7: What are the most common favourite seasons in the class?
C1_optD <- C1survey %>% 
  mutate(
    fav_season = str_to_title(str_squish(as.character(fav_season)))  # "fall" -> "Fall"
  )

season_counts <- C1_optD %>%
  filter(!is.na(fav_season), fav_season != "") %>%
  count(fav_season, sort = TRUE)
